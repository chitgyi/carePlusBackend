...
<div>
    <div class="my-4 flex items-center justify-between">
        <ol class=" list-reset flex text-grey-dark">
            ...
            <li>
                <p class="text-grey-600">Articles</p>
            </li>
        </ol>
        <div>
    ...
    <div class="bg-gray-100 box rounded">
        ...
        @foreach ($articles as $key => $article)
            <div class="grid grid-cols-5 p-3 border-b">
                <div class="grid grid-cols-4 col-span-">
                    <div>{{ $key + 1 }}</div>
                    <div class="flex items-center col-span-3 justify-center">
                        @if ($article->image)
                            <img class="w-24 h-20 rounded-md" src="{{ $article->image }}"
                                alt="{{ $article->title }}">
                        @endif
                    </div>
                </div>
                <div class="text-left">{{ $article->title }}</div>
                <div class="col-span-2 line-clamp-3 text-left">{{ $article->content }}</div>
